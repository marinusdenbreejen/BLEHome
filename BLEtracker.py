    ...
    for name, node in nodes.items():
        if name in measurements:
            ...
            if "floors" in node and room_history and room_history[-1] in node["floors"]:
                floor_weight = 1.5
            weight = floor_weight / max(distance, 0.1)
...
def build_mqtt_payload(stable_room, est_pos, smoothed_pos, accuracy, timestamp, used_nodes, device_name, device_id):
...
def process_tracking_cycle(client):
    ...
    device_id = next((d.get("id", "unknown") for d in node_measurements.values() if d), "unknown")
    ...
    if now - last_publish_time >= publish_interval:
        try:
            ...
            payload = build_mqtt_payload(stable_room, est_pos, smoothed_position, accuracy, timestamp, used_nodes, device_name, device_id)
